# -*- coding: utf-8 -*-
"""
Generate scene_stories.json in GBK encoding containing historical story descriptions for each of the 18 scenes.
"""
...
OUT_PATH = BASE_DIR / "assets" / "scene_stories.json"
...
STORIES = {
    ...
    "taiping": "至正十五年秋，朱元璋率军横渡长江，一举攻克太平府。进城后，朱元璋严令部下‘秋毫无犯’，严禁抢掠百姓，并开仓放粮，救济饥民。这一仁义之举与残暴的元军形成了鲜明对比，太平府的名士儒生和黎民百姓无不箪食壶浆，夹道欢迎王师。在这里，朱元璋不仅赢得了民心，还招揽了大量治国理政的优秀人才。太平府的和平占领，标志着朱元璋的军队正式从一支农民起义军，转变为有纪律、有抱负的仁义之师。",
    ...
    "zhedong": "至正十八年，朱元璋挥师东进，平定浙东地区。浙东自古多才俊，是江南文化的中心。朱元璋在此展现了对知识分子的极高尊重，亲自迎请了以刘基（刘伯温）、宋濂、章溢、叶琛为首的‘浙东四先生’。这些顶尖智囊的加入，彻底改变了朱元璋集团的知识结构，为他制定了极其严密的政治、军事和法律制度。浙东的收复，不仅扩大了统治疆域，更在精神和舆论上确立了朱元璋作为华夏正统继承者的地位。",
    ...
    "yunnan": "洪武十四年（1381年），大明开国已过十余载，但退守云南的元朝梁王势力依然负隅顽抗。朱元璋派遣大将傅友德、蓝玉、沐英率三十万大军远征云南。大军翻山越岭，荡平梁王余孽，收复云南全境。大将沐英因战功卓著，且忠心耿耿，被朱元璋委以重任，世代镇守云南。沐英不负重托，在云南兴修水利、屯田开荒、传播华夏文化，使西南边疆永固。至此，大明一统乾坤，海内升平，万世太平！"
}

def main():
    # Save to file using GBK encoding
    try:
        # We dump to a string first to ensure formatting, then write with GBK
        json_str = json.dumps(STORIES, ensure_ascii=False, indent=2)
        with open(OUT_PATH, "w", encoding="gbk") as f:
            f.write(json_str)
        print(f"Successfully generated {OUT_PATH.relative_to(BASE_DIR)} in GBK encoding!")
    except Exception as e:
        print(f"Error generating stories JSON: {e}")
